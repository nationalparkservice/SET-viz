...
FuncCalcQuant <- function(dat, unit) {
  ...
  dat2 <- dat %>%
    ...
    dplyr::mutate(Level = ifelse(
      unit < LowerCutoff, "below", ifelse(
        unit > UpperCutoff, "above", "between(inclusive)"))) %>%
...
FuncCalcThresh <- function(dat, unit) {
  ...
  dat2 <- dat %>% 
    ...
    Level = ifelse(
      unit < LowerCutoff, "below", ifelse(
        unit > UpperCutoff, "above", "between(inclusive)")))
  dat2$Level <- factor(dat2$Level, levels = c("below", "between(inclusive)", "above"))
  names(dat2)[names(dat2) == "unit"] <- unit
  return(dat2)
}
...
FuncPlotlyScatter <- function(dat, x_nam = "Wk", trans_y = "identity", add_loess = TRUE, rotate_x = FALSE) {
  ...
  # Create the plot(s)
    ...
    p1_points <- p1_points +
      scale_y_continuous(trans = trans_y, labels = function(x) as.character(round(x,2)), breaks = pretty_breaks(), limits = c(yaxes_min, yaxes_max)) + # alternatively, can use 'coord_cartesian(ylim = c(axes_min, axes_max))' to zoom in to the limits rather than "cutting off" the limits, as setting limits within scale_y_continuous() would do 
...
observeEvent(eventExpr = input$button_ImportCSV, {
  ...
  observeEvent(input$button_goSave, {
    ...
    cat("line725")
    write_csv(rv$DataSET, paste0("Data_out/", input$out_prefix, ".csv"))
...
observeEvent(eventExpr = input$button_RecalcThresholds, { 
  ...
  withProgress(message = "Just a moment...", detail = "...calculating thresholds", value = 0, {
    ...
    rv$ThreshDataSET <- FuncCalcQuant(dat = subset(rv$SubDataSET, SiteName == input$sel_HeatSite), unit = input$sel_CalcUnit)
    }
    cat("line772")
    ...
    if(input$sel_ThresholdType == "numb") {
      ...
      rv$ThreshDataSET <- FuncCalcThresh(dat = subset(rv$SubDataSET, SiteName == input$sel_HeatSite), unit = input$sel_CalcUnit)
    }
    cat("line777")
    # Create heat plots
  ...
  heat_legend <- grid.arrange(ggpubr::as_ggplot(
    get_legend(
      ggplot(rv$ThreshDataSET, aes(x = EventDate, y = PinPosition, fill = Level)) +
        geom_tile() + 
        scale_fill_manual(values = discrete_cols, na.translate = FALSE, drop = FALSE, name = ifelse(input$sel_CalcUnit == "ChangeMMPerYear", "Threshold Category (CHANGE PER YEAR):  ", "Threshold Category (CHANGE SINCE PRIOR):   ")) + 
