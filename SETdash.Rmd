...
### PICK UP FROM HERE
# Get CheckFLAGS to work
# Records Export Page 
# >> side panel allows user to choose which columns to view (and export). 
# >> User can edit CheckFLAGS and CHeckNote columns here. (User can use filter columns to show only flagged records or only above/below threshold records...)--this is where the Save-as-csv option should be, not at the data import stage.
# >> Button to export records in NERRS format
# User can click a cell on the heatmaps page to flag a record and add a CheckNOTES--click again to unflag. Cell text should be bold and larger when flagged.
# I think I should allow user to save the CheckNOTES and CheckFLAGS data in .RDS for later processing.
...
```{r startup}
rv <- reactiveValues(DataSET = NULL, CheckStatus = NULL, CheckCols = NULL, DataStation = NULL, UnitPoly = NULL, SubDataSET = NULL, SubDataSETRaw = NULL, SubDataSummary = NULL, PlotData = NULL, ShowDM = FALSE, ThreshDataSET = NULL, ThreshCounts = NULL, PickList = NULL, ThreshHeatPlots = NULL, ThreshMap = NULL)
...
FuncImportList <- function(filepaths_vec, is_SET = FALSE, colnames_start = "Park,Park_Code,Site_Name") {
  ...
  if(is_SET) {
    combined_file %<>% # convert all column names to upper camel style
      ...
      dplyr::mutate(
        CheckFLAG = FALSE,
        CheckNOTES = NA, # add a notes column for user to edit in dashboard
        RecordID = row_number()) %>% # add a unique ID to each record
      dplyr::select(CheckFLAG, CheckNOTES, RecordID, everything()) # arrange column order
    combined_file$CheckNOTES <- as.character(combined_file$CheckNOTES) # otherwise will assign it class of logical
  }
...
FuncShinyCol <- function(func_type, pref, RecordID_vec, default_val) {
  # Function to create a vector-list of shiny input checkboxes
  #
  # Args:
  #   func_type:  Shiny input function, e.g., checkboxInput
  #   pref:  The prefix to use for input ID
  #   RecordID_vec:  Vector of the record ID numbers
  #   default_val:  Value to use if no user input found
  #
  # Returns:
  #   A shiny vector in list format
  #
  cat("FUNCSHINYCOL")
  
  # Check current inputs for user-specified value, otherwise use default
  shinyValue = function(inputID, default = default_val) {
    value = input[[inputID]]
    if (is.null(value)) default_val else value
  }
  
  inputs = character(length(RecordID_vec))
  for (i in seq_len(length(RecordID_vec))) {
    ID <- paste0(pref, RecordID_vec[i])
    inputs[i] = as.character(func_type(ID, label = NULL, value = shinyValue(inputID = ID)))
    }
  return(inputs)
  }
...
FuncUpdateCheckCols = function(input_edit, output_table, proxyDT) {
  ...
  if(j == 0) { # edit in CheckFLAG
    cat("CHANGE FLAG")
  #   rv$DataSET[rv$DataSET$RecordID==ID_change, "CheckFLAG"] <<- DT::coerceValue(v, rv$DataSET[rv$DataSET$RecordID==ID_change, "CheckFLAG"])
  #   # replaceData(proxyDT, output_table, resetPaging = FALSE)
  }
  # 
  # if(j == 1) { # edit in CheckNOTES
  rv$DataSET[rv$DataSET$RecordID==ID_change, "CheckNOTES"] <<- DT::coerceValue(v, rv$DataSET[rv$DataSET$RecordID==ID_change, "CheckNOTES"]) # update in the raw
  # replaceData(proxyDT, output_table, resetPaging = FALSE)
  # }
  cat("line283")
...
FuncPlotlyHeatPlot <- function(dat, discrete_cols = c("#ffd700", "#add8e6", "#ffa500"), discrete_levels = c("below", "between(inclusive)", "above")) {
 ...
 heat_station_list <- lapply(sort(unique(dat2$StationName)), function(i) {
    ...
    template <- subdat_pipe %>%
      ...
      left_join(subdat_pipe[c("PipeDirection", "PinPosition", "EventDate", "ChangeMMPerYear", "HoverLabel", "Level")], by = c("PinPosition", "EventDate")) %>%
...
FuncThreshMap <- function(discrete_cols = c("#ffd700", "#add8e6", "#ffa500"), discrete_levels = c("below", "between(inclusive)", "above")) {
  shiny::req(!is.null(rv$ThreshCounts), !is.null(rv$DataStation), !is.null(rv$UnitPoly))
  map_dat <- rv$ThreshCounts %>%
    dplyr::ungroup() %>%
    dplyr::select(SiteName, StationName, EventDate, Level, N) %>%
    ...
    left_join(unique(rv$DataStation[c("Park", "SiteName", "StationName", "Latitude", "Longitude")]), by = c("SiteName", "StationName"))
    
  out_map <- leaflet() %>%
    # addMapPane("pane_SET", zIndex = 250) %>% # always keep selected monitoring station points on "top" of map layers (highest zIndex)
    # addMapPane("pane_ParkUnit", zIndex = 210) %>%
    ...
    # addPolygons(data = rv$UnitPoly, group = "NPS units", color = "white", weight = 1, options = pathOptions(pane = "pane_ParkUnit")) %>% # park unit outline
    addScaleBar() %>%
    addMinicharts(
          lng = map_dat$Longitude,
          lat = map_dat$Latitude,
          time = map_dat$EventDate,
          type = "pie",
          transitionTime = 0,
          layerId = map_dat$StationName,
          chartdata = as.matrix(subset(map_dat, select = discrete_levels)),
          colorPalette = discrete_cols) %>%
    setView(lng = mean(map_dat$Longitude, na.rm = TRUE), lat = mean(map_dat$Latitude, na.rm = TRUE), zoom = min(RgoogleMaps::MaxZoom(lonrange = range(map_dat$Longitude), latrange = range(map_dat$Latitude))))
    return(out_map)
}
...
observeEvent(eventExpr = input$button_ImportCSV, {
  ...
  withProgress(message = "Please wait a bit...", detail = "importing and formatting data", value = 0, {
    
    # Combine SET data in a single file
    unlink(list.files(path = here::here(), pattern = "^dataMaid"))
    temp_SET <- FuncImportList(filepaths_vec = SET_files_vec, is_SET = TRUE) %>% dplyr::filter(!is.na(StationName) & !is.na(EventDate) & !is.na(PipeDirection) & !is.na(PinPosition)  & !is.na(PinHeightMm1)) # remove records without result recorded in these important columns >>>>>>>>>>>>>>>>>>> REDO WITH complete.cases
    ...
    shiny::req(!is.null(rv$DataSET))
    FuncParkBoundaries(park_vec = sort(unique(rv$DataSET$ParkCode)))
    
    # shiny::req(!is.null(rv$DataSET))
    # rv$CheckCols <- data.frame(
    #   RecordID = seq_len(max(rv$DataSET$RecordID, na.rm = TRUE)),
    #   CheckFLAG = FALSE,
    #   CheckNOTES = ""
    # )
    cat("line567")
...
withProgress(message = "Generating data check report...", value = 0, { # show progress bar
    ...
    check_sub <- rv$SubDataSET %>%
      dplyr::select(-SetDirectionNotes) %>%
...
observeEvent(eventExpr = input$button_RecalcThresholds, { 
  ...
  withProgress(message = "Un momento por favor...", detail = "...calculating thresholds", value = 0, {
    ...
    incProgress(1/4, detail = "...generating heat plots")
    ...
    incProgress(2/4, detail = "...generating summary pie charts")
    rv$ThreshCounts <- rv$ThreshDataSET %>%
    dplyr::select(RecordID, SiteName, StationName, EventDate, DiffDays, LowerCutoff, UpperCutoff, Level) %>%
    # dplyr::filter(EventDate != min(EventDate, na.rm = TRUE)) %>%
    dplyr::group_by(SiteName, StationName, EventDate, DiffDays, LowerCutoff, UpperCutoff, Level, .drop = FALSE) %>%
    dplyr::summarise(N = n()) %>%
    dplyr::mutate(HoverLabel = paste0("Station: ", StationName, "<br>Date: ", EventDate, "<br>Days Diff: ", DiffDays, "<br>Upper Cutoff: ", UpperCutoff, "<br>LowerCutoff", LowerCutoff, "<br>LEVEL: ", Level, "<br>COUNT: ", N))
    
    # Generate map
    incProgress(3/4, detail = "...generating map")
    rv$ThreshMap <- FuncThreshMap()
  })
...
renderUI({
  ...
  rv$SubDataSETRaw <- rv$SubDataSET %>%
    ...
    dplyr::select(-CheckFLAG, -Yr, -Mnth, -DiffMMHeight, -DiffDays, -ChangeMMPerYear) %>%
    dplyr::mutate(
      CheckFLAGEntry = as.character(checkboxInput(inputId = "UserFlag", label = NULL, value = FALSE))) %>%
    dplyr::select(CheckFLAGEntry, CheckNOTES, RecordID, everything())

  # shiny::req(!is.null(rv$CheckCols))
  # cat("UPDATING...")
  #  lapply(seq_len(max(rv$DataSET$RecordID, na.rm = TRUE)), function(i) {
  #    cat("RECORD ", i)
  #   rv$CheckCols$CheckFLAG[rv$CheckCols$RecordID == i] <- input[[paste0("UserFLAG_", i)]]
  #   rv$CheckCols$CheckNOTES[rv$CheckCols$RecordID == i] <- input[[paste0("UserNOTES_", i)]]
   # }
   # )

# Update data frame of CheckCols ----
# renderUI({
#   shiny::req(!is.null(rv$CheckCols))
#    lapply(seq_len(max(rv$DataSET$RecordID, na.rm = TRUE)), function(i) {
#     observeEvent(input[[paste0("UserFLAG_", i)]], {
#       rv$CheckCols$CheckFLAG[rv$CheckCols$RecordID == i] <- input[[paste0("UserFLAG_", i)]]
#     })
#      observeEvent(input[[paste0("UserNOTES_", i)]], {
#       rv$CheckCols$CheckNOTES[rv$CheckCols$RecordID == i] <- input[[paste0("UserNOTES_", i)]]
#     })
#    })
})
...
```{r}
actionButton("button_ImportCSV", "Import Data from CSV Files", width = "100%", style="color:black; background-color: lightgreen; display:inline-block; border:2px; margin:5px")
```


```{r}
actionButton("button_ImportSQL", "Import Data from the SQL Database", width = "100%", style="color:black; background-color: lightgreen; display:inline-block; border:2px; margin:5px")
```
...
### Selected Raw Data
```{r selected_raw_data}
output$out_TableRaw = DT::renderDataTable({
  FuncOrderCheck(import = TRUE, examine = TRUE, plot = FALSE)
  shiny::req(!is.null(rv$SubDataSETRaw))

  cbind(
  CheckFLAGBox = FuncShinyCol(func_type = checkboxInput, pref = "UserFLAG", RecordID_vec = rv$SubDataSETRaw$RecordID, default_val = FALSE),
    rv$SubDataSETRaw)},
  server = FALSE,
  escape = FALSE,
  filter = "top",
  rownames = FALSE,
  selection = "none",
  editable = list(target = "cell", disable = list(columns = c( 2:ncol(rv$SubDataSETRaw)+1))), # column count starts at zero so the CheckNOTES column is at 1.
  options = list(
    columnDefs = list(
...
#     rownames = FALSE,
#     selection = "none",
#     editable = list(target = "cell", disable = list(columns = which(names(rv$SubDataSETRaw) != "CheckNOTES"))), # column count starts at zero but since we are adding the initial CheckFLAG column, can just use this column count to edit the CheckNOTES column.
#     options = list(
#       columnDefs = list(
#         list(className = "dt-center", targets = "_all"))))
#   
#   temp_raw %>%
#     formatStyle(c("CheckNOTES"), backgroundColor = "yellow") # highlight yellow the editable column
  # })
...
```{r}
    ...
    # }
# 
# shinyUpdateValue = function(pref, RecordID_vec) {
#       unlist(lapply(seq_len(length(RecordID_vec)), function(i) {
#         value = input[[paste0(pref, RecordID_vec[i])]]
#         if (is.null(value)) NA else value
#       }))
#     }
# 
# renderPrint({
#   rv$CheckStatus <- data.frame(
#     RecordID = 1:90,
#     CheckFLAG = shinyValue(id = "UserFLAG_", len = 90),
#     CheckNOTES = shinyValue(id = "UserNOTES_",  len = 90))
# })
```
...
FuncPlotlyPieMatrix <- function(dat, discrete_cols = c("#ffd700", "#add8e6", "#ffa500"), discrete_levels = c("below", "between(inclusive)", "above")) {
  ...
  for(s in 1:length(unique(dat$StationName))) {
    ...
    for(d in 1:length(unique(dat$EventDate))) { # first date is always NA's for change in pin height
...
output$out_ThreshMap <- renderLeaflet({
  ...
  FuncOrderCheck(import = TRUE, examine = TRUE, plot = TRUE)
  shiny::req(!is.null(rv$ThreshMap))
  rv$ThreshMap
})

tags$style(type = "text/css", "#out_ThreshMap {height: calc(100vh - 80px) !important;}") # expand map to fill screen

leafletOutput('out_ThreshMap', height="100%", width = "100%")
```

Flagged Data Records
====================================== 
```{r flagged_data}
# renderUI({
#   FuncOrderCheck(import = TRUE, filter = FALSE, plot = FALSE)
...
#   })
# 
# div(style="display:inline-block; width:100px; margin-right:10px;", actionButton("button_CategAddRow", "Add a Row"))
# div(style="display:inline-block; width:180px;", actionButton("button_CategDeleteRow", "Delete Selected Row(s)"))
# 
# br()
# 
# br()
# 
# output$categDT <- DT::renderDT({rv_param_temp$categLevelsPlot}, editable = TRUE)
