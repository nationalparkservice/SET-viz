...
marker_horizon_data <- DBI::dbGetQuery(con, marker_horizon_data_SQL)
  ...
  for(x in dat_vec) {
    ...
    rv$DataSET <- as.data.frame(FuncCalcChange(dat = dat_list$pin_data)) 
...
<font size="3">**These line plots show the selected SET HEIGHTS (raw pin, standardized pin, or orthometric) over time for the selected site.**</font>
...
renderPlotly({
  ...
  metric_text <- switch(rv$sel_SETmetric,
                      ...
                      standardized_soil_elev_mm = "STANDARDIZED pin height (mm)",
