...
FuncPlotlyMatrixPlot <- function(dat, heat_metric, discrete_cols = thresh3_cols, discrete_levels = thresh3_levels) {
  ...
  #
    ...
    if(heat_metric == "combined_base") { # for some reason, using ifelse within the mutate statement gives incorrect results
      dat2 %<>% dplyr::mutate(combined_change_height = paste0(diff_height_base_mm, "  [", pin_height_mm, "]"))
    } else {
        dat2 %<>% dplyr::mutate(combined_change_height = paste0(diff_height_mm, "   [", pin_height_mm, "]"))
      }
    
    dat2$level <- factor(dat2$level, levels = c("big decline", "no large changes", "big increase", "N/A"))
    
    dat2$level[is.na(dat2$level)] <-"N/A" # this is necessary so heatmap will still work even if no data 

  # Calculate total and relative page/plot heights
  temp_distinct <- dat2 %>% 
    dplyr::select(station_name, SET_direction_azimuth, pin_position) %>%
    dplyr::group_by(station_name, SET_direction_azimuth) %>%
  ...
  # Calculate total and relative subplot widths
   ...
   # incProgress(1/(length(unique(dat$station_name))+3), detail = paste0("...building heat plots for ", i))
    ...
    # create number matrix to put values in the heatmap cells
      ...
      expand(pin_position, event_date_UTC) %>%
        left_join(subdat_pipe[c("SET_direction_azimuth", "SET_direction", "pin_position", "event_date_UTC", "pin_height_mm", "diff_height_mm", "diff_height_base_mm", "combined_change_height", "hover_label", "level", "border_color")], by = c("pin_position", "event_date_UTC")) %>%
 ...
 final_heat_plotly <- # put the station plots together
   plotly::subplot( # setting margin to increase spacing btwn subplots still has the same problem of inner plots being differently sized--this is a known issue since 2016
     ...
     heights = rel_hts,
     which_layout = 1,
    margin = c(0, 0.0, 0.02/length(unique(temp_distinct$station_name)), 0.14/length(unique(temp_distinct$station_name))))
...
FuncOrderCheck <- function(import = TRUE, examine = TRUE) {
  ...
  if(examine == TRUE) {
    ...
    shiny::validate(
      need(!is.null(rv$SubDataSET), message = "Please first use the 'PARK SUMMARY' tab to choose a park and a subset of data to examine."))
    shiny::validate(
      need(!is.null(rv$SubDataSummary), message = "Please first use the 'PARK SUMMARY' tab to choose a park and a subset of data to examine."))
  }
}
...
marker_horizon_data <- DBI::dbGetQuery(con, marker_horizon_data_SQL)
  ...
  for(x in dat_vec) {
    ...
    showModal(modalDialog(
      title = "Done",
      HTML("The formatted data have been saved as '", input$out_prefix, ".RDS' (for future dashboard use) in the 'Data_out' folder of the current working directory, ", getwd(), ". The original (raw) tables imported from SQL have been saved as .csv files in the 'Data_in' folder of the current working directory. <br><br>You may now navigate to the PARK SUMMARY tab to run Data Check reports.") 
    ))
...
cat("line1184")
  ...
  showModal(modalDialog(
  title = "Done",
  "You may now navigate to the PARK SUMMARY tab to select a Park Unit."
  ))
}
...
PARK SUMMARY - Outputs {data-width=300 .tabset .tabset-fade}
-------------------------------------
...
# >>>>>>>>>>> PICK UP FROM HERE. FOR EACH SUBPLOT, SHOW THE ARM (A,B,C,D) AND DIRECTION. CAN REMOVE STATION NAME B/C THAT IS ALREADY IN TITLE. SEE IF I CAN SHOW DATE (X-AXIS FOR EACH SUBPLOT)
renderUI({
