...
FuncCalcChange <- function(dat) {
  ...
  dt1 <- dat %>%
    ...
    dplyr::select(record_ID, park_code, site_name, station_name, station_code, SET_direction_azimuth, pin_position, event_date_UTC, pin_height_mm, SET_reader, SET_instrument, pin_collection) %>%
    dplyr::arrange(event_date_UTC)
  
  dt2 = data.table(dt1 %>% dplyr::select(-record_ID))
  dt2[, c("event_date_UTC_lag", "pin_height_mm_lag", "SET_reader_lag", "SET_instrument_lag", "pin_collection_lag") := shift(.SD, 1), by=c("park_code", "site_name", "station_name", "station_code", "SET_direction_azimuth", "pin_position")] # lag calculations
  ...
  init_surveys <- dt1 %>%
   ...
   slice_min(order_by = event_date_UTC) %>%
    dplyr::select(park_code, site_name, station_name, station_code, SET_direction_azimuth, pin_position, first_event_date_UTC = event_date_UTC, first_pin_height_mm = pin_height_mm)
...
FuncCalcPinAlert <- function(dat, event_level_dat) {
  ...
  cat("line252")
  dat2 <- dat %>%
    # dplyr::select(record_ID, site_name, station_name, SET_direction_azimuth, SET_direction, pin_position, event_date_UTC, observation_type, pin_position, pin_height_mm, dpl, dpl_note, pin_flag_label, pin_flag_notes, vegetation_notes, SET_notes, exclude_int_reason_ID, exclude_int_reason, exclude_cumul_reason_ID, exclude_cumul_reason, SET_reader_change, SET_reader_transition, SET_instrument_change, SET_instrument_transition, pin_collection_change, pin_collection_transition) %>%
...
FuncCalcQuant <- function(dat) {
  ...
  if(rv$sel_HeatMetric %in% c("change", "combined")) {
    dat2$level <- ifelse(
      ...
      dat2$diff_height_mm > dat2$upper_cutoff, "big increase", "no large changes"))
  }
...
FuncCalcThresh <- function(dat) {
  # Function to classify diff_height_mm or diff_height_base_mm by user-specified numeric thresholds
  #
  # Args:
   ...
   incProgress(1/2, detail = "...creating subplots")
    ...
    # create number matrix to put values in the heatmap cells
      ...
      expand(pin_position, event_date_UTC) %>%
        left_join(subdat_pipe[c("SET_direction", "SET_direction_azimuth", "pin_position", "event_date_UTC", "pin_height_mm", "diff_height_mm", "diff_height_base_mm", "combined_change_height", "hover_label", "level", "border_color")], by = c("pin_position", "event_date_UTC")) %>%
    ...
    txt <- matrix(pull(template, 
                       switch(heat_metric,
                              "raw" = pin_height_mm, 
                              "change" = ifelse(heat_metric == "combined_base", diff_height_base_mm, diff_height_mm),
...
withProgress(message = "Just a moment", detail = "...generating data check report", value = 0, { # show progress bar
  ...
  if(input$sel_Pin_or_MH == "pin") {
    check_sub <- rv$SubDataSET %>%
      dplyr::select(-record_ID, -pin_flag_notes, -vegetation_notes, -SET_direction_notes, -SET_notes, -dpl_note, -event_date_mnth, -event_date_wk, -SET_reader_transition, -pin_collection_transition, -diff_days_base)
  } else {
    check_sub <- rv$SubDataMarker %>%
      dplyr::select(-record_ID, -marker_horizon_name, -core_measurement_number, -core_event_notes, -core_notes, -event_date_mnth, -event_date_wk, -reader_change)
  }
...
<font size="3">**This report summarizes data range/frequency and % missing for every numeric and categorical variable in the raw SET PIN or Marker Horizon data.**</font>

* Click the 'Run Data Check' button to generate a report for the selected Park, year range, and data type (SET pin or marker horizon).
...
```{r data_check_report}
shiny::addResourcePath("working_dir", here::here())

renderUI({
  FuncOrderCheck(import = TRUE, examine = TRUE)
  cat("line1842")
  shiny::validate(need(rv$ShowDM == TRUE, message = "Select a Park, then click the green 'Run Data Check' button (may take a little time to run; progress bar is below)"))
...
renderUI({
      ...
      radioButtons("sel_HeatMetric",
                   label = strong("Text to show on heatmaps:"),
                   choiceNames = list("Change in SET pin height [SET pin height]", "Change in SET pin height FROM START [SET pin height]"),
                   choiceValues = list("combined", "combined_base"),
                   # choiceNames = list("SET pin height", "Change in SET pin height", "Change in SET pin height [SET pin height]", "Change in SET pin height FROM START [SET pin height]"),
                   # choiceValues = list("raw", "change", "combined", "combined_base"),
                   selected = "combined")
      })
...
renderUI({
  shiny::req(!is.null(input$sel_Park), !is.null(rv$PinHeatDataSET), !is.null(input$sel_PinSite), !is.null(rv$sel_PinSite), !is.null(rv$SubDataSET), !is.null(input$sel_StationCode), !is.null(rv$PinHeatPlots))
  shiny::req(rv$sel_PinSite == input$sel_PinSite, unique(rv$PinHeatDataSET$park_name)==input$sel_Park)
  cap_text <- switch(rv$sel_HeatMetric,
                     "raw" = "SET pin heights",
                     "change" = "CHANGE in SET pin heights",
                     "combined" = "CHANGE in SET pin heights from PRIOR survey [SET pin heights are in brackets]",
                     "combined_base" = "CHANGE in SET pin heights from FIRST survey [SET pin heights are in brackets]")
...
renderPlot({
  ...
  if(all(unique(rv$PinHeatDataSET$park_name)==input$sel_Park, rv$sel_PinSite == input$sel_PinSite)) { # plot should turn blank if user has selected another park or site and this figure has not updated to reflect that change
    legend_text <- switch(rv$sel_HeatMetric,
                        "combined" = "CHANGE in SET pin heights from PRIOR survey ",
                        "combined_base" = "CHANGE in SET pin heights from FIRST survey ")
