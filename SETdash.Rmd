...
FuncShinyCol <- function(func_type, pref, RecordID_vec, ...) {
  ...
  for (i in seq_len(length(RecordID_vec))) {
    inputs[i] = as.character(func_type(paste0(pref, RecordID_vec[i]), label = NULL, ...))
    }
  return(inputs)
}
...
observeEvent(eventExpr = input$button_GetData, {
...
<font size="4">2.  Click the green 'Get Data' button at the bottom of this page.</font>
...
```{r}
actionButton("button_GetData", "Get Data", width = "100%", style="color:black; background-color: lightgreen; display:inline-block; border:2px; margin:5px")
```
...
output$table_raw = DT::renderDataTable({
  ...
  cbind(
    CheckFLAG = FuncShinyCol(func_type = checkboxInput, pref = "UserFlag_", RecordID_vec =  rv$SubDataSETRaw$RecordID, value = FALSE),
    CheckNOTES = FuncShinyCol(func_type = textInput, pref = "UserNotes_", RecordID_vec =  rv$SubDataSETRaw$RecordID, value = ""),
    rv$SubDataSETRaw)},
  server = FALSE,
  escape = FALSE,
  filter = "top",
