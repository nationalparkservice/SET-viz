...
FuncPlotlyMHHeatPlot <- function(dat, heat_metric, discrete_cols, discrete_levels) {
  ...
  # total_page_ht <- max(64 + nrow(temp_distinct) * (30*mean(temp_distinct$n_cores)), 300)
...
FuncOrderCheck <- function(import = TRUE, examine = TRUE) {
  ...
  if(examine == TRUE) {
    ...
    shiny::validate(
      need(!is.null(rv$SubDataSET), message = "Please first use the 'SET SUMMARY' tab to choose a park and a subset of data to examine."))
    shiny::validate(
      need(!is.null(rv$SubDataSummary), message = "Please first use the 'SET SUMMARY' tab to choose a park and a subset of data to examine."))
  }
}
...
marker_horizon_data <- DBI::dbGetQuery(con, marker_horizon_data_SQL)
  ...
  for(x in dat_vec) {
    ...
    showModal(modalDialog(
      title = "Done",
      HTML("The formatted data have been saved as '", input$out_prefix, ".RDS' (for future dashboard use) in the 'Data_out' folder of the current working directory, ", getwd(), ". The original (raw) tables imported from SQL have been saved as .csv files in the 'Data_in' folder of the current working directory. <br><br>You may now navigate to the SET SUMMARY tab to run Data Check reports.") 
    ))
...
cat("line1184")
  ...
  showModal(modalDialog(
  title = "Done",
  "You may now navigate to the SET SUMMARY tab to select a Park Unit."
  ))
}
...
SET SUMMARY - Outputs {data-width=300 .tabset .tabset-fade}
-------------------------------------
...
renderPlotly({
  ...
  plot_dat <- rv$SubDataSET %>% 
    ...
    dplyr::select(station_name, SET_direction_azimuth, SET_direction, pin_position, event_date_UTC, pin_height_mm, pin_flag_label, pin_flag_notes) %>%
    dplyr::arrange(station_name, SET_direction, pin_position, event_date_UTC) %>%
    dplyr::mutate(
      ...
      unique_pin = paste0(station_name, "_", SET_direction, " (", SET_direction_azimuth, " degrees)", pin_position),
        hover_text = paste0("<span style='font-size:16px; font-weight:bold;'>", unique_arm, "<br>Pin ", pin_position, "</span><br>Date: ", event_date_UTC, "<br>Pin Height(mm): ", pin_height_mm, ifelse(!is.na(pin_flag_label), paste0("<br>Pin Flag: ", pin_flag_label, "<br>Pin Flag Notes: ", pin_flag_notes), ""))
      )
  ...
  ggplotly(p, tooltip = c("text"), height = length(unique(plot_dat$station_name)) * 350, width = 800) %>%
    ...
    add_annotations( # shared y-axis title
      ...
      text = "Pin Height (mm)",
          ...
          textangle=-90,
          font=list(size=18, family = "Arial")) %>%
  highlight(on = "plotly_click", off = "plotly_doubleclick", opacityDim = 0.35)
})
```
