...
FuncCalcPinAlert <- function(dat, event_level_dat) {
  ...
  dat2 <- dat %>%
    dplyr::mutate(
      ...
      sel_diff_metric = get(paste(rv$sel_DiffFromBase, rv$sel_HeatMetric, sep = "_"))
      )
...
FuncPlotlyMatrixPlot <- function(dat) { 
  ...
  color_scale <- data.frame(z=z,col=rep(use_cols, each = 2)) 
    ...
    plot_ly(
      data = template, 
      text = text,
      width = max(900, ifelse(rv$sel_HeatMetric == "soil_elev_m", 105, 90)*length(unique(template$event_date_UTC))),
      height = total_page_ht, # can't specify heights in subplot, need to set it at this plot_ly level
      showscale = FALSE) %>%
      plotly::add_heatmap(
        x = ~as.factor(event_date_UTC),
    ...
    title_metric_text <- switch(rv$sel_HeatMetric,
...
tags$div(renderText({
  ...
  renderUI({
  shiny::req(!is.null(input$sel_ThresholdType), !is.null(input$sel_HeatMetric))
  conditionalPanel(
    condition = "(input.sel_ThresholdType == 'numb')",
    numericInput("sel_ThresholdLower",
                 label = strong("Enter lower limit"),
                 max = 0,
                 value = ifelse(input$sel_HeatMetric == "soil_elev_m", -0.01, -20)),
...
renderUI({
  ...
  shiny::req(rv$sel_PinSite == input$sel_PinSite, unique(rv$PinHeatDataSET$park_name)==input$sel_Park)
    ...
    metric_text <- switch(rv$sel_HeatMetric,
...
renderPlotly({
  ...
  shiny::req(!is.null(rv$sel_HeatMetric))
...
<font size="3">**These line plots show the marker horizon depths over time for the selected site.**</font>
...
renderPlotly({
  ...
  p <- ggplot(plot_dat, aes(x = event_date_UTC, y = avg_reading, text = hover_text)) +
    geom_line(aes(group = marker_horizon_name)) + 
