...
# Change per year--really it depends on season, so instead of this show some indication of season and then have a warning sign that the time btwn events is not necessarily approx annual
...
FuncPlotlyHeatPlot <- function(dat, discrete_cols = c("#ffd700", "#add8e6", "#ffa500"), discrete_levels = c("below", "between(inclusive)", "above")) {
 ...
 heat_station_list <- lapply(sort(unique(dat2$StationName)), function(i) {
    ...
    subdat <- dplyr::filter(dat2, StationName == i) %>%
      droplevels()
    ...
    pipe_dxn_list <- lapply(sort(unique(subdat$PipeDirection)), function(j) { # subplot the pipe directions for a station
      subdat_pipe <- dplyr::filter(subdat, PipeDirection == j)
  ...
  for(s in 1:length(unique(dat$StationName))) {
    ...
    for(d in 2:length(unique(dat$EventDate))) { # first date is always NA's for change in pin height
      ...
      fig <- fig %>% 
        ...
        add_pie(data = dplyr::filter(dat, StationName == station & EventDate == survey_date),
                labels = ~Level, 
                values = ~N,
                textposition = 'inside',
...
renderUI({
  ...
  rv$SeasonalPlots <- FuncPlotlyScatter(dat = dplyr::filter(rv$SubDataSET, SiteName == input$sel_HeatSite))
  Sys.sleep(1)
})
...
observeEvent(eventExpr = input$button_RecalcThresholds, { 
  ...
  withProgress(message = "Just a moment...", detail = "...calculating thresholds", value = 0, {
    ...
    rv$ThreshDataSET <- FuncCalcQuant(dat = dplyr::filter(rv$SubDataSET, SiteName == input$sel_HeatSite), sel_unit = input$sel_CalcUnit)
    }
    cat("line772")
    ...
    if(input$sel_ThresholdType == "numb") {
      ...
      rv$ThreshDataSET <- FuncCalcThresh(dat = dplyr::filter(rv$SubDataSET, SiteName == input$sel_HeatSite), sel_unit = input$sel_CalcUnit)
    }
    cat("line777")
    # Create heat plots
