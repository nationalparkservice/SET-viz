...
FuncPlotlyMHHeatPlot <- function(dat, heat_metric, discrete_cols, discrete_levels) {
  ...
  #
    ...
    dat2 <- dat%>%
      ungroup() %>%
    dplyr::arrange(station_code, core_measurement_number, event_date_UTC) %>%
  ...
  color_scale <- data.frame(z=z,col=rep(use_cols, each = 2)) # the default color for NA is white
    ...
    plot_ly(
      ...
      layout(
        ...
        xaxis = list( # x-axis
          title = "Survey Date\n", # not enough space before next subplot. <br> does not work
          tickangle = 270,
    ...
    plotly::subplot(
      ...
      layout(
          margin = list(
          t=30,
          b=0, # this margin is for around the full plot page, not each subplot
          l= 80,
          r=40)
        )
 }) 
...
FuncPlotlyMatrixPlot <- function(dat, heat_metric, discrete_cols = thresh3_cols, discrete_levels = thresh3_levels) {
  ...
  dplyr::summarize(n_pins = length(unique(pin_position)))
...
observeEvent(eventExpr = input$button_RecalcMH, { 
  ...
  withProgress(message = "Just a moment", detail = "...calculating summaries", value = 0, {
    ...
    rv$RawMHHeatPlots <- FuncPlotlyMHHeatPlot(dat = rv$MarkerHeatDataSET, heat_metric = "raw", discrete_cols = c("#F0E442", "EEE9E9"), discrete_levels = c("MH_alert", "no_alert")) # yellow for alert, gray for no alert
    cat("line983")
...
renderPlotly({ # separate validation statements so they are checked in sequence
  FuncOrderCheck(import = TRUE, examine = TRUE)
  shiny::validate(
      need(!is.null(rv$RawMHHeatPlots), message = "Please select a site and other filter criteria for the plots, then click the green 'Update Summaries' button"))
  shiny::req(!is.null(input$sel_Park), !is.null(rv$MarkerHeatDataSET))
