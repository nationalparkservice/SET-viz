...
### QUESTIONS
# pin_positionB --is that column name correct?
# check if I identified the correct cols to be factor, integer, date formats
# Removing records with NA in any of these columns (station_name, visit_date, SET_direction_azimuth, pin_positionB, pin_height_mm)--any reason to keep those in? Can provide an output of which records were excluded, or just a summary of counts
...
FuncCalcChange <- function(dat) {
  ...
  #
  dat$temp_visit_date <- dat$visit_date # need this to calculate DIffDays correctly (skipping the NA dates, but still need visit_date to order the readings correctly)
  dat$temp_visit_date[is.na(dat$pin_height_mm)] <- NA
  temp_dat <- dat %>%
    dplyr::group_by(park_name, park_code, site_name, station_name, SET_direction_azimuth, pin_positionB) %>%
...
FuncPlotlyScatter <- function(dat, x_nam = "Wk", trans_y = "identity", add_loess = TRUE, rotate_x = FALSE) {
  ...
  # Create the plot(s)
    ...
    p_points <- ggplot(data = dat, aes(x = Week, y = pin_height_mm, text = paste0("Site: ", site_name, "<br>Station: ", station_name, "<br>Visit Date: ", visit_date, "<br>X-value: ", Week, "<br>Y-value: ", pin_height_mm))) +
    ...
    # if(add_loess) {
      p1_points <- p1_points + geom_smooth(data = dat, aes(x = Week, y = pin_height_mm), method = "loess", size = 0.5, alpha = 0.2, show.legend = FALSE)
      # }
...
FuncPlotlyHeatPlot <- function(dat, discrete_cols = c("#ffd700", "#add8e6", "#ffa500"), discrete_levels = c("below", "between(inclusive)", "above")) {
  ...
  dat2 <- dat %>%
    ungroup() %>%
    dplyr::select(record_ID, site_name, station_name, SET_direction_azimuth, pin_positionB, visit_date, pin_height_mm, observation_type, PinFlag, diff_height_mm, diff_days, change_per_year_mm, lower_cutoff, upper_cutoff, level) %>%
    dplyr::arrange(record_ID) %>%
    dplyr::mutate(hover_label = paste0("Record ID: ", record_ID, "<br>Visit Date: ", visit_date, "<br>Observation Type: ", observation_type, "<br>Pin Position: ", pin_positionB, "<br>Pin Height (mm): ", pin_height_mm, "<br>Height Diff (mm): ", diff_height_mm, "<br>Days Diff: ", diff_days, "<br>Upper Cutoff: ", upper_cutoff, "<br>Lower Cutoff", lower_cutoff, "<br>Change Per Yr (mm): ", change_per_year_mm, "<br>LEVEL: ", level)) %>%
    droplevels()
  
  # Calculate total and relative heights
  temp_distinct <- dat2 %>% 
    dplyr::select(station_name, SET_direction_azimuth, pin_positionB) %>%
    distinct()
 ...
 heat_station_list <- lapply(sort(unique(dat2$station_name)), function(i) {
    ...
    template <- subdat_pipe %>%
      dplyr::select(pin_positionB, visit_date) %>%
      expand(pin_positionB, visit_date) %>%
      left_join(subdat_pipe[c("SET_direction_azimuth", "pin_positionB", "visit_date", "change_per_year_mm", "diff_height_mm", "hover_label", "level")], by = c("pin_positionB", "visit_date")) %>%
      dplyr::arrange(visit_date, pin_positionB)
    txt <- matrix(pull(template, input$sel_CalcUnit), nrow=length(unique(template$pin_positionB)))
  ...
  color_scale <- data.frame(z=z,col=rep(use_cols, each = 2)) # the default color for NA is white
    ...
    plot_ly(
      ...
      add_annotations( 
        ...
        y = as.integer(template$pin_positionB),
        align = "center",
...
FuncFormatSET_NERRs <- function(dat) {
  temp_NERRs <- dat %>%
    ...
    dplyr::mutate(
      ...
      arm_qaqc_code = NA,
      pin_number = paste0("pin_", pin_positionB),
...
observeEvent(eventExpr = input$button_ImportCSV, {
  ...
  withProgress(message = "Just a moment", value = 0, {
    ...
    integer_cols <- c("SET_offset_mm", "pin_length_mm", "pin_height_mm", "standardized_soil_elev_mm") # <<<<<<<<<<<<<<<<<<<<<< CHECK THESE
    
    temp_pin2 <- temp_pin %>% # for easier filtering
      dplyr::mutate_at(vars(matches(factor_cols)), as.factor) %>%
      dplyr::mutate_at(vars(matches(integer_cols)), as.integer) %>%
      dplyr::mutate_at(vars(matches("_date")), lubridate::as_date, format = "%m/%d/%Y")
      dplyr
# <<<<<<<<<<<<< PICK UP FROM HERE. MAKE SURE COL NAMES MATCH. CHANGE URL FOR GETTING PARK BOUNDARIES, ETC.
  ...
  if(!is.null(rv$DataSET)) {
    rv$DataSET %<>% 
      dplyr::mutate(
        ...
        Mnth = factor(lubridate::month(visit_date), levels = 1:12),
...
observeEvent(eventExpr = input$button_RecalcThresholds, { 
  ...
  withProgress(message = "Just a moment...", detail = "...calculating thresholds", value = 0, {
    ...
    rv$MapDat <- rv$ThreshCounts %>%
      ...
      left_join(unique(rv$DataStation[c("park_name", "park_code", "site_name", "station_name", "Latitude", "Longitude")]), by = c("site_name", "station_name"))
    })
  cat("line805")
saveRDS(isolate(reactiveValuesToList(rv)), "TEMP_rv.RDS")
  # Sys.sleep(2)
...
renderUI({
  ...
  dplyr::filter(park_name == input$sel_Park & Yr >= as.integer(input$sel_YrRange[1]) & Yr <= as.integer(input$sel_YrRange[2]) & DataProcessingLevel %in% input$sel_ProcessLevel & ObservationType1 %in% input$sel_ObservationType) 
...
renderUI({
  ...
  rv$SubDataSETRaw <- rv$SubDataSET %>%
    ...
    dplyr::select(-Yr, -Mnth, -Week, -diff_height_mm, -diff_days, -change_per_year_mm)
  }
  ...
  wellPanel(
    ...
    renderUI({
      ...
      sliderInput(
        ...
        min = min(rv$DataSET$Yr[rv$DataSET$park_name == input$sel_Park], na.rm = TRUE), 
        max = max(rv$DataSET$Yr[rv$DataSET$park_name == input$sel_Park], na.rm = TRUE), 
        step = 1, 
        sep="", 
        value = c(min(rv$DataSET$Yr[rv$DataSET$park_name == input$sel_Park], na.rm = TRUE), max(rv$DataSET$Yr[rv$DataSET$park_name == input$sel_Park], na.rm = TRUE)), 
        dragRange = TRUE, 
        width = "85%")
      }),
...
renderPlot({
  ...
  heat_legend <- grid.arrange(ggpubr::as_ggplot(
    get_legend(
      ggplot(rv$ThreshDataSET, aes(x = visit_date, y = pin_positionB, fill = level)) +
        geom_tile() + 
