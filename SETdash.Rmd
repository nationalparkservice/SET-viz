...
observeEvent(eventExpr = input$button_ImportSQL, {
  ...
  shiny::validate(need(!is.null(input$database_server), message = "Enter a valid SQL Database Server"))
...
marker_horizon_data <- DBI::dbGetQuery(con, marker_horizon_data_SQL)
  ...
  for(x in dat_vec) {
    ...
    showModal(modalDialog(
      title = "Done",
      HTML("The formatted data have been saved as '", input$out_prefix, ".RDS' (for future dashboard use) in the 'Data_out' folder of the current working directory, ", getwd(), ". The original (raw) tables imported from SQL have been saved as .csv files in the 'Data_in' folder of the current working directory. <br><br>You may now navigate to the BIG PICTURE SUMMARY tab to run Data Check reports.") 
    ))
...
observeEvent(eventExpr = input$button_RecalcPinHeights, { 
  ...
  shiny::req(!is.null(rv$SubDataSET), !is.null(input$sel_PinHeightSite),  !is.null(input$sel_ThresholdType))
...
renderPlotly({ # separate validation statements so they are checked in sequence
  FuncOrderCheck(import = TRUE, examine = TRUE)
  shiny::validate(
      need(!is.null(rv$PinHeatPlots), message = "Please first enter data filter criteria for the plots, then click the green 'Update Summaries' button"))
cat("line1716")
...
output$out_PieCharts <- renderPlotly({
  FuncOrderCheck(import = TRUE, examine = TRUE)
  shiny::req(!is.null(rv$ThreshCounts), !is.null(input$sel_PinHeightSite))
  ...
  FuncPlotlyPieMatrix(dat = rv$ThreshCounts, plot_title = paste0(input$sel_PinHeightSite, ": Change in Pin Height from Prior Survey (each row is a Station, each column is a Survey Event)"))
})
