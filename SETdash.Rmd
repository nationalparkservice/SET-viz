 ...
 heat_station_list <- lapply(sort(unique(dat2$station_name)), function(i) {
  ...
  color_scale <- data.frame(z=z,col=rep(use_cols, each = 2)) # the default color for NA is white
    ...
    plot_ly(
      data = template, 
      text = text,
      width = max(500, 52*length(unique(template$event_date_UTC))), 
...
FuncPlotlyMatrixPlot <- function(dat, heat_metric, discrete_cols = thresh3_cols, discrete_levels = thresh3_levels) {
  ...
  #   heat_metric: "raw" for raw pin data, "change" for change in pin height since last survey, or "combined" for both pieces of information
  #   discrete_cols: For discrete fill, the vector of colors
  #   discrete_levels: For discrete fill, the levels corresponding with colors
  #   
  # Returns:
 ...
 heat_station_list <- lapply(sort(unique(dat2$station_name)), function(i) {
   ...
   # incProgress(1/(length(unique(dat$station_name))+3), detail = paste0("...building heat plots for ", i))
    ...
    # create number matrix to put values in the heatmap cells
      ...
      expand(pin_position, event_date_UTC) %>%
        left_join(subdat_pipe[c("SET_direction_azimuth", "SET_direction", "pin_position", "event_date_UTC", "pin_height_mm", "diff_height_mm", "combined_change_height", "hover_label", "level", "border_color")], by = c("pin_position", "event_date_UTC")) %>%
  ...
  color_scale <- data.frame(z=z,col=rep(use_cols, each = 2)) 
    ...
    plot_ly(
      data = template, 
      text = text,
      width = max(800, ifelse(heat_metric == "combined", 100*length(unique(template$event_date_UTC)), 52*length(unique(template$event_date_UTC)))),
      height = total_page_ht, # can't specify heights in subplot, need to set it at this plot_ly level
      showscale = FALSE) %>%
      plotly::add_heatmap(
        x = ~as.factor(event_date_UTC),
...
cat("line1034")
    ...
    shiny::req(!is.null(rv$PinHeatDataSET), !is.null(input$sel_HeatMetric))
cat("line1040")

    incProgress(1/3, detail = paste0("...creating heat plots"))
    
    rv$PinHeatPlots <- FuncPlotlyMatrixPlot(dat = rv$PinHeatDataSET, heat_metric = input$sel_HeatMetric)
    cat("988")
    ...
    rv$MapDat <- rv$ThreshCounts %>%
      ...
      dplyr::filter(event_date_UTC != min(rv$ThreshCounts$event_date_UTC, na.rm = TRUE)) %>% # First Sampling date is all NA's so omit
...
1.  Enter your SQL DRIVER and the SQL DATABASE SERVER.
...
PARK SUMMARY - Outputs {data-width=100 .tabset .tabset-fade}
-------------------------------------
...
output$out_TableSummary <- renderReactable({
  ...
  reactable(
    ...
    columns = list(
      ...
      min_yr = colDef(header = with_tooltip("First Sampling", "Earliest year with survey data"), minWidth = 80, align = "center"),
      max_yr = colDef(header = with_tooltip("Last Sampling", "Most recent year with survey data"), minWidth = 80, align = "center"),
...
<font size="3">**Raw PIN data for the selected Park and year range.**</font>
...
renderUI({
      ...
      radioButtons("sel_HeatMetric",
                   ...
                   choiceNames = list("Pin height", "Change in pin height", "Change in pin height [pin height]"),
                   choiceValues = list("raw", "change", "combined"),
                   selected = "combined")
      })
...
<font size="3">**This heatmap shows PIN HEIGHTS or CHANGE IN PIN HEIGHTS for the selected site, color-coded to highlight large increases or declines. Red borders on heatmap cells indicate presence of selected alerts.**</font>

<span style="font-size:18px; color:red;">
    INTERACTIVE HEAT PLOTS ARE SLOW TO DRAW - PLEASE BE PATIENT 
</span>

* Only showing pin readings classified as "Standard" (not "Double Read") data.
...
renderPlot({
  shiny::req(!is.null(input$sel_Park), !is.null(rv$PinHeatDataSET), !is.null(rv$PinHeatPlots))
  
  if(unique(rv$PinHeatDataSET$park_name)==input$sel_Park) { # plot should turn blank if user has selected another park and this figure has not updated to reflect that change
...
renderUI({
  ...
  cap_text <- switch(rv$sel_HeatMetric,
                     "raw" = "actual pin heights",
                     "change" = "CHANGE in pin heights",
                     "combined" = "CHANGE in pin heights [actual pin heights are shown in brackets]")
    div( HTML(paste0("<br><center><text style='color:blue; font-size:22px; font-weight:bold;'>", toupper(unique(rv$PinHeatDataSET$park_name)), ":   ", rv$sel_PinSite, " (Yrs ", input$sel_YrRange[1], "-", input$sel_YrRange[2], ")</text><br><text style='color:blue; font-size:18px;'>Values in heatmap cells are ", cap_text, "</text></center><br>")) )
  })
...
cat("line1716")
shiny::req(!is.null(input$sel_Park), !is.null(rv$PinHeatDataSET))
shiny::validate(need(unique(rv$PinHeatDataSET$park_name)==input$sel_Park, message = "Park selection has changed. Please click the green 'Update Summaries' button to refresh plots."))
...
renderUI({
  ...
  shiny::req(rv$sel_PinSite %in% unique(rv$SubDataSET$site_name), unique(rv$PinHeatDataSET$park_name)==input$sel_Park)
    ...
    div( HTML(paste0("<br><center><text style='color:blue; font-size:22px; font-weight:bold;'>", toupper(unique(rv$PinHeatDataSET$park_name)), ":   ", rv$sel_PinSite, " (Yrs ", input$sel_YrRange[1], "-", input$sel_YrRange[2], ")</text><br><text style='color:blue; font-size:18px;'>Trends in actual pin heights</text></center><br>")) )
  })

renderPlotly({
  FuncOrderCheck(import = TRUE, examine = TRUE)
  shiny::validate(
      need(!is.null(rv$sel_PinSite), message = "Please select a site and other filter criteria for the plots, then click the green 'Update Summaries' button"))
    
    shiny::validate(need(rv$sel_PinSite %in% unique(rv$SubDataSET$site_name), message = "Park selection has changed. Please click the green 'Update Summaries' button to refresh plots."))
...
renderUI({
  ...
  shiny::req(rv$sel_PinSite %in% unique(rv$SubDataSET$site_name), unique(rv$PinHeatDataSET$park_name)==input$sel_Park)
    ...
    div( HTML(paste0("<br><center><text style='color:blue; font-size:22px; font-weight:bold;'>", toupper(unique(rv$PinHeatDataSET$park_name)), ":   ", rv$sel_PinSite, " (Yrs ", input$sel_YrRange[1], "-", input$sel_YrRange[2], ")</text><br><text style='color:blue; font-size:18px;'>% of Station-Survey data in each pin height change category</text></center><br>")) )
  })

renderPlot({
  shiny::req(!is.null(rv$PinHeatDataSET), !is.null(rv$PinHeatPlots))
...
output$out_PieCharts <- renderPlotly({
  ...
  shiny::req(!is.null(rv$ThreshCounts), !is.null(input$sel_PinSite))
  
      shiny::validate(
      need(!is.null(rv$ThreshCounts), message = "Please select a site and other filter criteria for the plots, then click the green 'Update Summaries' button"))
      
          shiny::validate(need(rv$sel_PinSite %in% unique(rv$SubDataSET$site_name), message = "Park selection has changed. Please click the green 'Update Summaries' button to refresh plots."))
      
  
  FuncPlotlyPieMatrix(dat = rv$ThreshCounts, plot_title = paste0(input$sel_PinSite, ": Change in Pin Height from Prior Survey (each row is a Station, each column is a Survey Event)"))
})
...
renderUI({
  ...
  shiny::req(rv$sel_PinSite %in% unique(rv$SubDataSET$site_name), unique(rv$PinHeatDataSET$park_name)==input$sel_Park)
    ...
    div( HTML(paste0("<br><center><text style='color:blue; font-size:22px; font-weight:bold;'>", toupper(unique(rv$PinHeatDataSET$park_name)), ":   ", rv$sel_PinSite, " (Yrs ", input$sel_YrRange[1], "-", input$sel_YrRange[2], ")</text><br><text style='color:blue; font-size:18px;'>Time lapse map showing % of data in each pin height change category, by Station</text></center><br>")) )
  })
...
output$out_ThreshMap <- renderLeaflet({
  ...
  FuncOrderCheck(import = TRUE, examine = TRUE)
  shiny::validate(
      need(!is.null(rv$MapDat) & !is.null(input$sel_MapZoom) & !is.null(input$sel_MapPieSize), message = "Please select a site and other filter criteria for the plots, then click the green 'Update Summaries' button"))
  
  shiny::req(!is.null(input$sel_Park))
shiny::validate(need(unique(rv$MapDat$park_name)==input$sel_Park, message = "Park selection has changed. Please click the green 'Update Summaries' button to refresh plots."))
...
shiny::validate(need(unique(rv$MarkerHeatDataSET$park_name)==input$sel_Park, message = "Park selection has changed. Please click the green 'Update Summaries' button to refresh plots."))
