...
### FIX
# Heat plot legend should not update until update summaries button is pressed
# Speed up action when updating heat plots--subplots and annotations are very slow in plot_ly
...
# INclude these data processing levels--if I only choose Accepted and there are no such data, then it should show blank (but doesn't)
...
rv <- reactiveValues(DataSET = NULL, DataStation = NULL, DataEvent = NULL, DataCriticalInfo = NULL, DataMarker = NULL, UnitPoly = NULL, SubDataSET = NULL, SubDataSummary = NULL, SubDataDateSummary = NULL, PlotData = NULL, ShowDM = FALSE, ThreshDataSET = NULL, ThreshCounts = NULL, PickList = NULL, RawHeatPlots = NULL, ThreshHeatPlots = NULL, SeasonalPlots = NULL, MapDat = NULL, DataSET_NERRs = NULL)
...
# 
out <- readRDS("~/NPS PROJECTS_CURRENT/Dash_SETviz/Data_out/SET_20220907.RDS")
...
FuncCalcEventAlert <- function(dat) {
  # Function to create table of event alerts
  #
  # Args:
...
observeEvent(eventExpr = input$button_ImportSQL, {
  ...
  for(x in dat_vec) {
    ...
    rv$DataMarker <- dat_list$marker_horizon_data
...
renderUI({
 cat("line1087")
  shiny::req(!is.null(rv$DataSET), !is.null(rv$DataEvent), !is.null(rv$DataCriticalInfo), !is.null(input$sel_Park), !is.null(input$sel_YrRange[1]), !is.null(input$sel_YrRange[2]), !is.null(input$sel_ProcessLevel), !is.null(input$sel_ObservationType))
       cat("line1089")
  ...
  rv$SubDataEvent <- rv$DataEvent %>%
    dplyr::filter(
      park_name == input$sel_Park & lubridate::year(event_date_UTC) >= as.integer(input$sel_YrRange[1]) & lubridate::year(event_date_UTC) <= as.integer(input$sel_YrRange[2])) # only including events where pin data collected
...
<font size="3">**This report summarizes data range/frequency and % missing for every numeric and categorical variable in the raw PIN data.**</font>

* Click the 'Run Data Check' button to generate a report for the selected Park and year range.
...
renderUI({
  ...
  selectInput(
    ...
    selected = switch(is.null(input$sel_PinHeightSite)+1, input$sel_PinHeightSite, sort(unique(rv$SubDateSET$site_name))[1])
        )
      })
...
<span style="font-size:18px; color:red;">
    INTERACTIVE PLOTS ARE SLOW TO DRAW - PLEASE BE PATIENT 
</span>
