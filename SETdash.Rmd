...
FuncSubplotMargins <- function(tot_page, num_subplots, left_prop = 0, right_prop = 0, top_prop = 0.1, bottom_prop = 0.1) {
  adj_top = ifelse(tot_page > 400, top_prop/num_subplots, .06) # need to divide so as number of plots increases, the space between plots doesn't get huge
  adj_bottom = ifelse(tot_page > 400, bottom_prop/num_subplots, .06)
...
FuncPlotlyMHHeatPlot <- function(dat, heat_metric, discrete_cols, discrete_levels) {
  ...
  total_page_ht <- max(100 + 30*sum(temp_distinct$n_cores), 350) # <<<<<<<<<<<<<<<<<<<<<<
  cat("line523")
  ...
  cat("line557")
    ...
    final_MHheat_plotly <- plotly::subplot(
      ...
      add_annotations( # Station name as title
        x = 0.5,
        y = 1.1,
        text = paste0("Marker horizon data (averaged across cores) for ", unique(subdat$station_name)),
        showarrow = F,
        xref="paper",
        yref="paper",
        xanchor="center",
  ...
  color_scale <- data.frame(z=z,col=rep(use_cols, each = 2)) 
    ...
    final_heat_plotly <- plotly::subplot(
      ...
      add_annotations( # Station name as title
        x = 0.5,
        y = 1.02,
        text = paste0("<br><br>SET pin data (uncorrected for pin length) for ", unique(subdat$station_name),"<br><br>"),
        showarrow = F,
        xref="paper",
        yref="paper",
        xanchor="center",
...
renderUI({
  ...
  cap_text <- switch(rv$sel_HeatMetric,
                     ...
                     "combined" = "CHANGE in SET pin heights [SET pin heights are in brackets]",
...
<font size="3">**These line plots show SET PIN HEIGHTS over time for the selected site.**</font>
...
<font size="3">**These pie charts show large increases or declines in SET pin heights (i.e., change in SET pin heights since prior survey) as % of the data for a survey event.**</font>
...
<font size="3">**This map shows the CHANGE IN SET PIN HEIGHTS  (i.e., change in SET pin heights since prior survey) as pie charts overlaid on a map of the selected site.**</font>
...
renderUI({
  ...
  sliderInput("sel_MapZoom", label = "Set map zoom (smaller = wider view):", min = map_zoom - 3, max = map_zoom + 3, value = map_zoom - 1, step = 0.5, ticks = FALSE, dragRange = FALSE, width = "95%")
}),
...
renderUI({
  ...
  shiny::req(rv$sel_MHSite %in% unique(rv$SubDataSET$site_name), unique(rv$MarkerHeatDataSET$park_name)==input$sel_Park)
    ...
    div( HTML(paste0("<br><center><text style='color:blue; font-size:22px; font-weight:bold;'>", toupper(unique(rv$MarkerHeatDataSET$park_name)), ":   ", rv$sel_MHSite, " (Yrs ", input$sel_YrRange[1], "-", input$sel_YrRange[2], ")</text><br><text style='color:blue; font-size:18px;'>Values shown are average marker horizon depth per core ID</text></center><br>")) )
  })
