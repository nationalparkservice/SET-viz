...
thresh3_levels = c("big decline", "within expected range", "big increase", "N/A")
...
FuncCalcQuant <- function(dat) {
  ...
  if(rv$sel_HeatMetric == "combined_base") {
    dat2$level <- ifelse(
      ...
      dat2$diff_height_base_mm > dat2$upper_cutoff, "big increase", "within expected range"))
  }
...
FuncPlotlyMatrixPlot <- function(dat, heat_metric, discrete_cols = thresh3_cols, discrete_levels = thresh3_levels) {
  ...
  #
    ...
    dat2$level <- factor(dat2$level, levels = c("big decline", "within expected range", "big increase", "N/A"))
...
cat("line1034")
  ...
  rv$PinPieMatrix <- FuncPlotlyPieMatrix(dat = rv$ThreshCounts, plot_title = paste0(rv$sel_PinSite, ": Change in SET Pin Height from Prior Survey (each row is a Station, each column is a Survey Event)"))
    ...
    first_survey <- min(rv$ThreshCounts$event_date_UTC, na.rm = TRUE)
    rv$MapDat %<>%
      dplyr::filter(event_date_UTC != first_survey) %>% # First Sampling date is all NA's so omit. This is more steps than should be necessary, but didn't seem to work all the time when piped
      droplevels() %>%
...
<font size="3">**This heat map shows RAW SET PIN HEIGHTS and/or CHANGE IN RAW SET PIN HEIGHTS (since prior survey) for the selected site, color-coded to highlight large increases or declines. Red borders on heatmap cells indicate presence of selected alerts.**</font>
...
renderUI({
  ...
  yr_range <- rv$PinHeatDataSET %>% dplyr::filter(station_code == input$sel_StationCode) %>% pull(event_date_yr) %>% range()
    div( HTML(paste0("<br><center><text style='color:blue; font-size:22px; font-weight:bold;'>", toupper(unique(rv$PinHeatDataSET$park_name)), ":   ", rv$sel_PinSite, " ", input$sel_StationCode, " (Yrs ", yr_range[1], "-", yr_range[2], ")</text><br><text style='color:blue; font-size:18px;'>Values in heatmap cells are ", cap_text, "</text></center><br>")) )
  })
...
renderUI({
  ...
  shiny::req(all(rv$sel_PinSite == input$sel_PinSite,  unique(rv$PinHeatDataSET$park_name)==input$sel_Park))
    ...
    div(HTML(paste0("<br><center><text style='color:blue; font-size:22px; font-weight:bold;'>", toupper(unique(rv$PinHeatDataSET$park_name)), ":   ", rv$sel_PinSite, " (Yrs ", rv$sel_YrRange[1], "-", rv$sel_YrRange[2], ")</text><br><text style='color:blue; font-size:18px;'>Trends in SET pin heights</text></center><br>")) )
  })
...
<font size="3">**This map shows the CHANGE IN SET PIN HEIGHTS pie charts overlaid on a map of the selected site.**</font>
