...
# >>>>>>>>>>>>>>>>> TESTING
# out <- readRDS("~/NPS PROJECTS_CURRENT/Dash_SETviz/Data_out/SET_20221103_ALL.RDS")
...
FuncCalcPinAlert <- function(dat, event_level_dat) {
  ...
  cat("line252")
  dat2 <- dat %>%
    # dplyr::select(record_ID, site_name, station_name, SET_direction_azimuth, SET_direction, pin_position, event_date_UTC, pin_height_mm, observation_type, pin_position, pin_height_mm, dpl, dpl_note, pin_flag, pin_flag_notes, vegetation_notes, SET_notes, exclude_int_reason_ID, exclude_int_reason, exclude_cumul_reason_ID, exclude_cumul_reason, SET_reader_change, SET_reader_transition, SET_instrument_change, SET_instrument_transition, pin_collection_change, pin_collection_transition) %>%
    ...
    dplyr::mutate(
      ...
      pin_flag_alert = if("pin_flag_alert" %in% input$sel_PinAlerts) {  ifelse(!is.na(pin_flag)|!is.na(pin_flag_notes), TRUE, FALSE)} else {FALSE},
  ...
  wellPanel(
    ...
    renderUI({
      ...
      checkboxGroupInput(
        ...
        selected = c("pin_flag_alert", "SET_notes_alert", "analysis_alert", "SET_instrument_alert", "pin_collection_alert", "event_notes_alert"))
   })
  )
