...
# I think I should allow user to save the CheckNOTES data in .RDS for later processing.
...
```{r startup}
rv <- reactiveValues(DataSET = NULL, CheckStatus = NULL, CheckCols = NULL, DataStation = NULL, UnitPoly = NULL, SubDataSET = NULL, SubDataSETRaw = NULL, SubDataSummary = NULL, PlotData = NULL, ShowDM = FALSE, ThreshDataSET = NULL, ThreshCounts = NULL, PickList = NULL, ThreshHeatPlots = NULL, MapDat = NULL)
...
FuncImportList <- function(filepaths_vec, is_SET = FALSE, colnames_start = "Park,Park_Code,Site_Name") {
  ...
  if(is_SET) {
    combined_file %<>% # convert all column names to upper camel style
      ...
      dplyr::select(CheckNOTES, RecordID, everything()) # arrange column order
...
shinyValue = function(inputID, default) {
    value = input[[inputID]]
...
observeEvent(eventExpr = input$button_ImportCSV, {
  ...
  withProgress(message = "Just a moment", value = 0, {
    ...
    incProgress(2/3, detail = "...getting park unit boundaries from IRMA")
...
observeEvent(eventExpr = input$button_RecalcThresholds, { 
  ...
  withProgress(message = "Just a moment...", detail = "...calculating thresholds", value = 0, {
    ...
    dplyr::select(RecordID, ParkCode, SiteName, StationName, EventDate, DiffDays, LowerCutoff, UpperCutoff, Level) %>%
    # dplyr::filter(EventDate != min(EventDate, na.rm = TRUE)) %>%
    dplyr::group_by(ParkCode, SiteName, StationName, EventDate, DiffDays, LowerCutoff, UpperCutoff, Level, .drop = FALSE) %>%
    dplyr::summarise(N = n()) %>%
    dplyr::mutate(HoverLabel = paste0("Park: ", ParkCode, "<br>Station: ", StationName, "<br>Date: ", EventDate, "<br>Days Diff: ", DiffDays, "<br>Upper Cutoff: ", UpperCutoff, "<br>LowerCutoff", LowerCutoff, "<br>LEVEL: ", Level, "<br>COUNT: ", N))
...
output$out_TableRaw = DT::renderDT({
  ...
  temp_raw <- DT::datatable(
    ...
    rownames = FALSE,
    selection = "none",
    editable = list(target = "cell", disable = list(columns = c( 1:ncol(rv$SubDataSETRaw)-1))), 
    options = list(
      autoWidth = TRUE,
      columnDefs = list(
  ...
  temp_raw %>% formatStyle("CheckNOTES", backgroundColor = "yellow")
})
...
renderUI({
  ...
  sliderInput("sel_MapZoom", label = "Set map zoom (smaller = wider view):", min = map_zoom - 3, max = 20, value = map_zoom - 1, step = 0.25, ticks = FALSE, dragRange = FALSE, width = "95%")
})
...
renderPlotly({ # separate validation statements so they are checked in sequence
  ...
  withProgress(message = "SO CLOSE!!", detail = "...drawing interactive plots", value = 0, {
...
output$out_ThreshMap <- renderLeaflet({
  ...
  shiny::req(!is.null(rv$MapDat), !is.null(rv$UnitPoly), !is.null(input$sel_MapZoom), !is.null(input$sel_MapPieSize))
