...
### PICK UP FROM HERE
# User can edit the CheckFlag and CheckNote columns in Raw Data File--I'm having trouble accessing the RecordID to make that change
# Records Export Page 
# >> side panel allows user to choose which columns to view (and export). 
# >> User can edit CheckFlag and CHeckNote columns here. (User can use filter columns to show only flagged records or only above/below threshold records...)--this is where the Save-as-csv option should be, not at the data import stage.
# >> Button to export records in NERRS format
# User can click a cell on the heatmaps page to flag a record and add a CheckNote--click again to unflag. Cell text should be bold and larger when flagged.
# I think I should allow user to save the CheckNotes and CheckFlag data in .RDS for later processing.
...
FuncImportList <- function(filepaths_vec, colnames_start = "Park,Park_Code,Site_Name") {
  ...
  combined_file <- plyr::rbind.fill(import_list) %>% # combine the data frames
    ...
    dplyr::mutate(RecordID = row_number()) %>% # add a unique ID to each record
    dplyr::select(RecordID, everything()) # put the 'RecordID' column first
...
FuncCalcChange <- function(dat) {
  ...
  temp_dat <- dat %>%
    ...
    dplyr::ungroup()
  temp_dat$CheckFlag = FALSE # so user can flag the record for export
  temp_dat$CheckNote = NA # so user can add notes for export
  
  return(temp_dat)
}
  
# FuncUpdateCheckCols = function(input_edit, output_edit, proxyDT) {
#   # Function to update the CheckFlag or CheckNote columns of rv$DataSET
#   #
#   # Args:
...
#   #   output_edit: The data frame to be updated, e.g., rv$SubDataSETRaw
#   #   proxyDT:  The name of the proxy data table, e.g., raw_proxyDT
#   #
#   # Returns:
#   #   Updates the output data frame and also rv$DataSET
#   #
#   info = input_edit
#   i = info$row
#   j = info$col
#   v = info$value
#   
#   output_edit[i, j] <<- DT::coerceValue(v, isolate(output_edit[i, j]))
#   replaceData(proxyDT, output_edit, resetPaging = FALSE)
#   ID_change <- output_edit$RecordID[i]
#   col_change <- names(output_edit)[j]
#   rv$DataSET[rv$DataSET$RecordID==ID_change, col_change] <- v
#   
#   saveRDS(isolate(reactiveValuesToList(rv)), "TEMP_rv.RDS")
# saveRDS(isolate(reactiveValuesToList(input)), "TEMP_input.RDS")
# }
...
# Update CheckFlag and CheckNote columns ----
# observeEvent(input$table_raw_cell_edit, {
#   FuncUpdateCheckCols(input_edit = input$table_raw_cell_edit, output_edit = rv$SubDataSETRaw, proxyDT = raw_proxyDT)
#   })
```
...
output$table_raw <- DT::renderDT({
  ...
  temp_raw <- DT::datatable(
    rv$SubDataSETRaw, # show only the original raw data columns, not newly calculated columns
    filter = "top",
    rownames = FALSE,
    selection = "none",
    editable = list(target = "cell", disable = list(columns = which(!names(rv$SubDataSETRaw) %in% c("CheckFlag", "CheckNote"))-1)), # column count starts at zero
    options = list(
      columnDefs = list(list(className = "dt-center", targets = "_all"))))
  temp_raw %>%
    formatStyle(c("CheckFlag", "CheckNote"), backgroundColor = "yellow") # highlight yellow the editable columns
})
