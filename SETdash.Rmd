...
### PICK UP FROM HERE
# got it--saved as input$Flag_(recordID) and input$Notes_recordID), with the latter being inputTExt. Need to use that information to update the subdatasetraw in real time
# User can edit the CheckFLAGS and CheckNOTES columns in Raw Data File--I'm having trouble accessing the RecordID to make that change
# Records Export Page 
...
```{r startup}
rv <- reactiveValues(DataSET = NULL, DataMarker = NULL, DataStation = NULL, SubDataSET = NULL, SubDataSETRaw = NULL, SubDataSummary = NULL, PlotData = NULL, ShowDM = FALSE, ThreshDataSET = NULL, PickList = NULL, ThreshHeatMaps = NULL)
...
FuncImportList <- function(filepaths_vec, colnames_start = "Park,Park_Code,Site_Name") {
  ...
  combined_file <- plyr::rbind.fill(import_list) %>% # combine the data frames
    ...
    dplyr::select(RecordID, everything()) 
...
FuncShinyCol <- function(func_type, pref, RecordID_vec, default_value, ...) {
  # Function to create a vector-list of shiny input checkboxes
  #
  # Args:
  #   func_type:  Shiny input function, e.g., checkboxInput
  #   init_value_vec:  The data class should be consistent with the shiny function
  #
  # Returns:
  ...
  for (i in seq_len(length(RecordID_vec))) {
    inputs[i] = as.character(func_type(paste0(pref, RecordID_vec[i]), label = NULL, value = switch(is.null(input[[paste0(pref, RecordID_vec[i])]])+1, input[[paste0(pref, RecordID_vec[i])]], default_value), ...)) # initial value is the default value unless it already has a prior value recorded 
    }
  return(inputs)
}

FuncShinyVals <- function(value_list, null_value) {
  # Function to extract values from a shiny vector in list format
  #
  # Args:
  #   id:   Prefix identifier for each vector element
  #   value_vec:  The vector of initial values. Should be as long as length of vector, and data class should be consistent with the shiny function
  #
  # Returns:
  #   A vector of data values
  #
  
  unlist(lapply(value_vec, function(i) {
        value = input[[i]]
        if (is.null(value)) null_value else value
        }))
  }
...
observeEvent(eventExpr = input$button_ImportCSV, {
  ...
  # setProgress(1/2, detail = "saving output files")
  ...
  #               placeholder = paste0("Sitename_", paste0(gsub(pattern = "-", replacement = "", x = Sys.Date())))
  #               ),
  #     footer = tagList(
...
# (Home) Save formatted files ----
# observeEvent(input$button_GoSave, {
#     saveRDS(isolate(reactiveValuesToList(rv)), paste0("Data_out/", input$out_prefix, ".RDS"))
#     write_csv(rv$DataSET, paste0(here::here(), "/Data_out/", input$out_prefix, "_SET_formatted.csv"))
#     write_csv(rv$DataStation, paste0(here::here(), "/Data_out/", input$out_prefix, "_station_formatted.csv"))
#   
#     showModal(modalDialog(
#       title = "Done",
#       paste0("The data for dashboard use have been saved as '", input$out_prefix, ".RDS' in the folder 'Data_out' of the current working directory, ", here::here(), ". For your reference, the formatted data have also been saved in .csv format as '", input$out_prefix, "_SET_formatted.csv' and '", input$out_prefix, "_station_formatted.csv' in the same 'Data_out' folder.You may now navigate to the 'Examine Data' tab of this dashboard.") 
#     ))
#   })
...
observeEvent(eventExpr = input$button_RunDataCheck, {
  ...
  shiny::req(!is.null(rv$SubDataSET), !is.null(rv$SubDataSummary), !is.null(input$table_summary_rows_selected)) 
...
observeEvent(eventExpr = input$button_RecalcHeatMaps, { 
  ...
  withProgress(message = "Calculating heat maps...", value = 0, {
    ...
    pick_list <- rv$ThreshDataSET %>% # pick-list for user clicks on heat map
...
# Update CheckNOTES columns ----
# observeEvent(input$table_raw_cell_edit, {
#   FuncUpdateCheckCols(input_edit = input$table_raw_cell_edit, output_table = rv$SubDataSETRaw, proxyDT = raw_proxyDT)
#   })
...
renderUI({
  ...
  rv$SubDataSETRaw <- rv$SubDataSET %>%
    ...
    dplyr::select(-Yr, -Mnth, -DiffMMHeight, -DiffDays, -ChangeMMPerYear)
})
```
...
Examine Data - Outputs {.tabset .tabset-fade}
-------------------------------------
...
output$table_raw = DT::renderDataTable({
  ...
  cbind(
    CheckFLAG = FuncShinyCol(func_type = checkboxInput, pref = "UserFlag_", RecordID_vec =  rv$SubDataSETRaw$RecordID, default_value = FALSE),
    CheckNOTES = FuncShinyCol(func_type = textInput, pref = "UserNotes_", RecordID_vec =  rv$SubDataSETRaw$RecordID, default_value = ""),
    rv$SubDataSETRaw)},
  server = FALSE,
  escape = FALSE,
  filter = "top",
  rownames = FALSE,
  selection = "none",
  # editable = list(target = "cell", disable = list(columns = c(0, 2:ncol(rv$SubDataSETRaw)))), # column count starts at zero so the CheckNOTES column is at 1.
  options = list(
    columnDefs = list(
...
#   })
# raw_proxyDT <- DT::dataTableProxy("table_raw")
...
shinyValue = function(pref) {
  unlist(lapply(seq_len(2000), function(i) {
        value = input[[paste0(pref, i)]]
...
renderPrint({
  data.frame(
    CheckFLAG = shinyValue(pref = "UserFlag_"),
    CheckNOTES = shinyValue(pref = "UserNotes_"))
})
```
